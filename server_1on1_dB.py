...
while True:
    ...
    print(f"[Server] : {input_str}")

    if input_str.lower() == "query":
        # 저장되어 있는 목록 조회
        ...
        print("The client requests a 'query' command. Result: :")
        print(response)

    elif input_str.lower().startswith("id "):
        # id로 조회
        target_id = int(input_str.split()[1])
    ...
    elif input_str.lower().startswith("name "):
        # name으로 조회
        target_name = input_str[5:].strip()
        # name^ 이후 전체 문자열 가지고오는 구문문
        cursor.execute(
        ...
        try:
            ...
            db.commit()
            conn.sendall(f"[Server] Information saved successfully: {name}\n".encode())
